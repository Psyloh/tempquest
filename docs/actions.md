...
## ActionObjectives (Quest `actionObjectives`)

`actionObjectives` are quest objectives whose logic is implemented in code and registered in `QuestObjectiveRegistry`.

They use the same data format as actions:
...
**Arguments:**
- `<count>` — Minimum number of nearby flowers (required)

**Example:**
```json
{ "id": "plantflowers", "args": ["10"] }
```
...
Completes when a player watched-attribute string equals the expected value.
...
**Example:**
```json
{ "id": "hasattribute", "args": ["someKey", "someValue"] }
```
...
Completes when the player has interacted at the specified coordinates.

**Arguments:**
- `<x,y,z>` — Coordinate string (required)
- `[onCompleteActionString]` — Optional action string to execute when this coordinate is interacted with for the first time (optional)

**Example:**
```json
{ "id": "interactat", "args": ["123,64,-10", "notify 'Done'"] }
```
...
Completes when the player has interacted with all listed coordinates. Shows progress as `have/need`.

**Arguments:**
- `<x,y,z>` — One or more coordinate strings (required)

**Example:**
```json
{ "id": "interactcount", "args": ["0,64,0", "1,64,0", "2,64,0"] }
```
...
Checks if an integer watched-attribute meets a condition. Can optionally fire actions when the condition is met.

**Arguments:**
- `<varName>` — Attribute key (required)
- `<operator>` — One of `=`, `==`, `!=`, `>`, `>=`, `<`, `<=` (required)
- `<value>` — Integer value to compare against (required)
- `[actionsOnComplete]` — Action string to execute once when condition becomes true (optional)

**Example:**
```json
{ "id": "checkvariable", "args": ["kills", ">=", "10", "notify 'Complete'"] }
```
...
**Example:**
```json
{ "id": "randomkill", "args": ["yourmod:quest-id"] }
```
...
Completes when the player walks a specified distance in meters.

**Arguments:**
- `<questId>` — Quest ID used for tracking (required)
- `<meters>` — Distance in meters (required)
- `[slot]` — Optional slot index when tracking multiple walk objectives (optional)

**Example:**
```json
{ "id": "walkdistance", "args": ["yourmod:quest-id", "250"] }
```
...
Completes when the current time of day matches the requested mode.
